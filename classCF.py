...
def mnrj_uod_similarity(common, ids, jds, u, v):
...
def mnrj_thes_similarity(common, ids, jds, u, v, thres_i, thres_j):
    intersection_cardinality = len(set.intersection(*[set(ids), set(jds)]))
    Iu = len(set(ids))
    Iv = len(set(jds))
    ...
    # d_rate = np.abs(common_u - common_v).sum()  # /common_u.shape()
    num = 0
    for i in range(common_u.size):
        if (common_u[i] >= thres_i and common_v[i] >= thres_j):
            num += 1
        elif (common_u[i] < thres_i and common_v[i] < thres_j):
            num += 1
    if intersection_cardinality == 0:
        return 0
    else:
